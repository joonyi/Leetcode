...
the little match girl has, please find out a way you can make one square by using up all
those matchsticks. You should not break any stick, but you can link them up, and each matchstick
...
class Solution:
    def makesquare(self, nums: List[int]) -> bool:
        ...
        perimeter = sum(nums)  # Perimeter of our square (if one can be formed
        possible_side =  perimeter // 4  # Possible side of our square
        ...
        sums = [0 for _ in range(4)] # This array represents the 4 sides and their current lengths
        ...
        def dfs(index):
            ...
            # (provided their remaining lengths are >= the size of the current matchstick)
            for i in range(4):
    ...
    def makesquare2(self, nums):
        ...
        L = len(nums)
        perimeter = sum(nums)  # Possible perimeter of our square
        possible_side = perimeter // 4  # Possible side of our square from the given matchsticks
...
nums = [1,1,2,2,2]
nums = [3,3,3,3,4]
print(Solution().makesquare2(nums))
